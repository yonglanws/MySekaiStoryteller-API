...
/**
 * parallel 导出收尾：多段 mp4 无损拼接 + 音频合流。
 *
 * 各段由同一编码器配置（同 crf/预设/分辨率/fps）产出，编码参数一致，
 * 用 concat demuxer + `-c:v copy` 直接拼接，不重编码、无质量损失。
 * 任一段文件缺失都直接抛错，由上层走回退，绝不产出半截视频。
 */
export async function concatSegmentsWithAudio(
  segmentPaths: string[],
  ...
  if (segmentPaths.length === 0) {
    throw new Error('concatSegmentsWithAudio: no segments')
  }
  for (const seg of segmentPaths) {
    if (!fs.existsSync(seg)) {
      throw new Error(`concatSegmentsWithAudio: segment file missing: ${seg}`)
    }
  }

  const listFile = `${outputPath}.concat-list.txt`
  try {
    // concat demuxer 的 list 语法里反斜杠是转义字符：Windows 路径必须转成
    // 正斜杠，否则第二段起全部解析失败（表现为输出只剩第一段的时长）。
    const escaped = segmentPaths
      .map((p) => `file '${p.replace(/\\/g, '/').replace(/'/g, "'\\''")}'`)
      .join('\n')
    await fs.promises.writeFile(listFile, escaped, 'utf-8')
    logger.info(`Concat list (${segmentPaths.length} segment(s)):\n${escaped}`)

    const totalBytes = segmentPaths.reduce((sum, p) => sum + fs.statSync(p).size, 0)
    const estimatedDuration = Math.max(totalBytes / (8000000 / 8), 10)
    ...
    const ffmpegArgs = [
      ...
      '-safe',
      '0',
      // 重新生成 PTS：各段容器 duration 与帧数的微小差异会在接缝处累积，
      // genpts 让输出按固定帧率连续排布，避免拼接处掉帧/重复
      '-fflags',
      '+genpts',
      '-i',
      listFile,
      ...
      '-map',
      '0:v:0',
      ...(audioPath && fs.existsSync(audioPath) ? ['-map', '1:a:0'] : []),
      '-c:v',
      'copy',
      '-r',
      String(fps),
